...
- **自然语言查数据**：自然语言 -> 意图识别 -> AgentScope data planner -> 结构化分析计划 -> SQL Harness 审批执行 -> MySQL 执行，包含 SQL 安全分析、执行错误修复、异常结果反思、自动补表和用户友好结果展示。
...
- **AgentScope 运行区**：`data` 主路由先进入 AgentScope data planner 生成结构化 `analysis_plan`，再回到 SQL Harness 完成审批和执行；AgentScope 不直接执行 SQL，也不接管权限、审计或评测。
...
当前实现已经把 `data` 路由前置到 AgentScope data planner。AgentScope 的职责是探索业务知识、可见表、字段语义和表关系，并提交结构化 `analysis_plan` 或 SQL 草稿。

可用运行区分为三类：

- `exploratory_analysis`：只读探索，不提交 SQL。
- `data_analysis`：当前主路由的数据分析规划入口，提交 `analysis_plan` 回到 SQL Harness。
- `complex_analysis`：复杂分析工作区，允许生成 SQL 草稿，但草稿仍然是 `draft_only`，不能直接执行。

`report_generation` 只读取已有 artifact/result，不查询新数据。`SkillRegistry` 只影响 prompt 和允许工具集合，不会引入 SQL 执行或绕过权限门禁。
...
当前主链路是 `Final Graph -> AgentScope data planner -> analysis_plan -> SQL Harness`。`chat` 和 `clarify` 仍由同一个 Final Graph 分支处理，完整 SQLReact 子图作为 harness 和兼容路径保留。
...
| `agentscope_data_planner` | AgentScope 先探索业务知识、可见表、字段语义和表关系，提交结构化 `analysis_plan` |
| `approve_analysis_plan` | 把 `analysis_plan` 转成可审批的 `complex_plan`，并做权限和计划校验 |
| `execute_analysis_plan` | 进入 SQL Harness，完成 SQL 生成、安全检查、权限检查、审批恢复、执行和本地 merge/report |
